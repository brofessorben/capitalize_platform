...
export default function ChatPage({ params }) {
    ...
    return (
      <div className={`flex ${align} gap-3`}>
        ...
        <div className="flex flex-col gap-1">
          ...
          {items.map((m, i) => (
            <div key={i} className={`${bubbleBase} ${sender === "ai" ? "bg-white border-gray-200" : sender === "vendor" ? "bg-blue-600 text-white border-blue-700" : sender === "host" ? "bg-amber-600 text-white border-amber-700" : "bg-emerald-600 text-white border-emerald-700"}`}>
              {m.text}
            </div>
          ))}
        </div>
  ...
  function Avatar({ meta, right = false }) {
    return <div className={`w-8 h-8 rounded-xl grid place-items-center ${right ? "" : ""} ${meta.color} text-white`}><span className="text-base" title={meta.label}>{meta.badge}</span></div>;
  }
  ...
  function getSenderMeta(sender) {
    return { label: "AI", badge: "🤖", color: "bg-gray-900" } || { label: "Vendor", badge: "🧑‍🍳", color: "bg-blue-600" } || { label: "Host", badge: "🧑‍💼", color: "bg-amber-600" } || { label: "Referrer", badge: "🧭", color: "bg-emerald-600" };
  }

  function formatTime(d) {
    return new Date(d).toLocaleString(undefined, { hour: "numeric", minute: "2-digit" });
  }
    ...
    <div className="min-h-dvh bg-white">
      <div className="border-b sticky top-0 bg-white/90 backdrop-blur">
        <div className="mx-auto max-w-4xl px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><div className="font-semibold leading-tight">Negotiation Chat</div><div className="text-xs text-gray-500">Lead: {lead_id}</div></div>
          </div>
          <div className="flex items-center gap-2">
            {ROLE_OPTIONS.map((r) => (
              <button key={r.id} onClick={() => setSender(r.id)} className={`px-3 py-1.5 text-sm rounded-full border transition ${sender === r.id ? `${r.color} text-white border-transparent` : "border-gray-300 hover:bg-gray-50"}`} title={r.label}><span className="mr-1">{r.badge}</span>{r.label}</button>
            ))}
          </div>
        </div>
      </div>
      <div className="mx-auto max-w-4xl px-4 py-4">
        <div ref={scrollerRef} className="h-[70vh] border rounded-2xl bg-gray-50 p-4 overflow-y-auto shadow-inner">
          ...
          <div className="space-y-4">{grouped.map((g, idx) => <MessageGroup key={idx} sender={g.sender} when={g.when} items={g.items} />)}</div>
        </div>
        <form onSubmit={sendMessage} className="mt-4 flex items-end gap-2">
          <textarea className="flex-1 border rounded-2xl p-3 min-h-[48px] max-h-40 resize-y focus:outline-none focus:ring-2 focus:ring-black/10" placeholder="Type a message… (Enter to send, Shift+Enter for new line)" value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={handleKeyDown} />
          <button disabled={sending || input.trim() === ""} className="shrink-0 px-5 h-12 rounded-2xl bg-black text-white font-medium disabled:opacity-50">{sending ? "Sending…" : "Send"}</button>
        </form>
        <div className="mt-2 text-xs text-gray-500">Pro tip: switch roles (Vendor/Host/Referrer) to speak as that party. AI replies with next best step.</div>
      </div>
    </div>
  );
}
